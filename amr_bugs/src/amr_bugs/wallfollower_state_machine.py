...
def Find_Wall(userdata):
    ...
    # Front laser range is less than clearnce, return wall found
    if userdata.front_data < userdata.clearance:
...
# Function to align the vehicle to the wall
def Align_Wall(userdata):

# Balancing the side sensors
    if (abs(userdata.ranges[7] - userdata.ranges[8])<= 0.03 and not (userdata.front_data < userdata.clearance )):

        userdata.velocity = (0, 0, 0)
        return 'stopped aligning'
    else:
...
# Function to follow the wall until obstacle is found
def Follow_Wall(userdata):


    if ((userdata.front_data < userdata.clearance) or ((userdata.ranges[6]< 0.2) or (userdata.ranges[1]< 0.2))):

        return 'stopped wall following'
...
# Handling convex corners(when side sensors go out of balance)
        ...
        if ((userdata.front_data < userdata.clearance) or ((userdata.ranges[6]< 0.08) or (userdata.ranges[1]< 0.08))):
            return 'stopped wall following'
        else:
...
def set_ranges(self, ranges):
    ...
    if self.userdata.direction == 1:
        ...
        self.userdata.front_data = min(self.userdata.ranges[3],self.userdata.ranges[4])
        print len(ranges)
    else:
...
def construct():
    ...
    with sm:
        ...
        # State machine with 3 states 'FINDWALL', 'ALIGNWALL', 'FOLLOWWALL'
        smach.StateMachine.add('FINDWALL',
                             PreemptableState(Find_Wall,
                                              input_keys=['front_data', 'clearance'],
                                              output_keys=['velocity'],
                                              outcomes=['found_obstacle']),
                             transitions={'found_obstacle': 'ALIGNWALL'})

        smach.StateMachine.add('ALIGNWALL',
                             PreemptableState(Align_Wall,
                                              input_keys=['ranges', 'clearance','front_data','direction'],
                                              output_keys=['velocity'],
                                              outcomes=['stopped aligning']),
                             transitions={'stopped aligning': 'FOLLOWWALL'})

        smach.StateMachine.add('FOLLOWWALL',
                             PreemptableState(Follow_Wall,
                                              input_keys=['ranges', 'clearance','front_data'],
                                              output_keys=['velocity'],
                                              outcomes=['stopped wall following']),
                             transitions={'stopped wall following': 'ALIGNWALL'})
    return sm